...
pub fn execute_attention(
            ...
            for i in 0..seq_length {
                ...
                for j in 0..d_k {
                    ...
                    for k in 0..seq_length {
                        attended_value = attended_value + (attention_weights[k] * v_head[k * d_k + j]);
                    }
    ...
    let qkv_flops = 3 * (batch_size as u64) * (seq_length as u64) * (d_model as u64) * (d_model as u64);
    let attention_flops = (batch_size as u64) * (num_heads as u64) * (seq_length as u64) * (seq_length as u64) * (d_k as u64);
    let output_flops = (batch_size as u64) * (seq_length as u64) * (d_model as u64) * (d_model as u64);
